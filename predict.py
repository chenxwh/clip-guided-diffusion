...
class Predictor(BasePredictor):
    ...
    def predict(
        ...
        assert (
            isinstance(display_frequency, int) and display_frequency > 0
        ), "display_frequency is a positive integer"
        clip_model = self.clip_model
        self.model_config.update({"timestep_respacing": str(timesteps)})

        # Load models
        model, diffusion = create_model_and_diffusion(**self.model_config)
        model.load_state_dict(
            torch.load("256x256_diffusion_uncond.pt", map_location="cpu")
        )
        ...
        make_cutouts = MakeCutouts(self.clip_size, cutn)
        ...
        for prompt in prompts:
            ...
            target_embeds.append(
                clip_model.encode_text(clip.tokenize(txt).to(self.device)).float()
            )
        ...
        for j, sample in enumerate(samples):
            ...
            if j % display_frequency == 0 or cur_t == -1:
                for k, image in enumerate(sample["pred_xstart"]):
                    yield checkin(j, k, image, out_path)
